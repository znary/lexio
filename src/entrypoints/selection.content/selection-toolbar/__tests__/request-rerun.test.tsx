...
describe("selection toolbar requests", () => {
  ...
  it("renders dictionary partials immediately but saves only the final structured result", async () => {
    const dictionaryRun = createDeferredPromise<BackgroundStructuredObjectStreamSnapshot>()
    streamBackgroundStructuredObjectMock.mockImplementation((
      _payload: unknown,
      options?: { onChunk?: (snapshot: BackgroundStructuredObjectStreamSnapshot) => void },
    ) => {
      options?.onChunk?.(createStructuredObjectSnapshot({ definition: "演" }))
      return dictionaryRun.promise
    })
    getOrCreateWebPageContextMock.mockResolvedValue(null)
    ...
    await waitFor(() => {
      expect(screen.getByTestId("translation-detailed").textContent).toContain("\"definition\":\"演\"")
    })
    expect(saveTranslatedSelectionToVocabularyMock).not.toHaveBeenCalled()

    await act(async () => {
      dictionaryRun.resolve(createStructuredObjectSnapshot({ term: "demonstrate", definition: "演示" }))
      await Promise.resolve()
    })

    await waitFor(() => {
      expect(saveTranslatedSelectionToVocabularyMock).toHaveBeenCalledWith(expect.objectContaining({
        sourceText: "demonstrate",
        translatedText: "演示",
      }))
    })
    expect(translateTextCoreMock).not.toHaveBeenCalled()
